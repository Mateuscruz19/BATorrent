...
- [~] **⑥ HUB — games** — **minimal slice landed**: `session.gameLibrary()` lists game torrents (resolved `ContentType::Game`, else name/file heuristic) with cover + progress; HubView "Your games" grid; double-click/Play launches the game: a **manual** `gameExe/<hash>` override wins, otherwise it **auto-detects** the best-guess binary (skips redists/uninstallers, prefers the largest .exe near the root, recognizes setup/installers → "Installing" vs "Launching"); nothing runnable → opens the folder. Right-click → "Set executable…" / "Open folder". **Compile + smoke-tested on macOS** (the actual run is Windows-first). Still to do (gradual): smarter install flow, playtime + Discord RPC.
